# Company Brain
> Last synced: April 17, 2026 (UI Redesign v2 — complete frontend rebuild: 8 parallel agents, 40 files changed, 4178 insertions. Split-screen command center, vendor orange dashboard, 14 shared components, Zod schemas, MSW/Vitest Layer 7, Framer Motion. All 8 QA layers green. Commit 83b6015 → origin/staging.)
> Owner: Vagish Kapila
...
- **Key Features**: SOV parsing (Excel/CSV/PDF/DOCX), G702/G703 PDF generation, Stripe Connect payments (ACH + card), lien waivers, email send, admin dashboard, AI assistant, QuickBooks integration (built, pending env vars), reconciliation, job completed tracking, 2-step onboarding (Company Setup + Meet ARIA), 90-day trial, Trust Score /763 (5 tiers), CA Lien Module §8202, join codes, vendor book, early pay 1.5%, ZIP repository, vendor dashboard (orange theme), role switcher (Contractor/Vendor), SOV trade auto-detection, LienDashboard (/lien route — all lien waivers across projects, KPI cards, download per doc)
- **UI v2 (Apr 17, 2026)**: Split-screen command center (55% financial / 45% orbital), 14 shared components (Badge, KPICard, MoneyDisplay, StatusChip, TradeDot, InlineAlert, PayAppRow, ProjectCard, ARIAStrip, EmptyState, CurrencyInput, RoleSwitcher, SidebarSection, VendorDetailPanel), Sidebar with project sections (Needs Attention / Active / Billing Soon / Archived), TopNav with role switcher, Dashboard hero row (Ready to Bill CTA), HubInbox table view with ARIA batch suggest + rejection chips, magic-upload.html + join.html light theme, Framer Motion animations throughout
...
4. ✅ **Apr 17 UI Redesign v2 complete** — 8 parallel agents, 40 files changed (4178 insertions). Commit `83b6015` pushed to origin/staging. All 8 QA layers green: 33/33 arch, 194/194 static, 4/4 mutations, 0 TS errors, Vite build clean, G702 math passing, 211/211 component tests (Layer 7), staging healthy. Ready for Layer 9 real-user testing before merge to main.
5. ⏳ **Run Layer 9 on staging** — `npm run test:real-user:staging` — confirm zero broken indicators. Then merge staging → main.
6. ⏳ **Investigate Railway staging not auto-deploying** — origin/staging had commit 945becb but Railway staging never picked it up. Check Railway staging service config (branch binding, build triggers). Until resolved, push directly to origin/main only after all QA layers pass locally.
...
| Apr 17, 2026 | UI Redesign v2 — complete frontend rebuild via 8 parallel agents | 40 files changed, 4178 insertions, 1807 deletions. Commit 83b6015 → origin/staging. All 8 QA layers green (33/33 arch, 194/194 static, 0 TS errors, 211/211 Vitest component tests). Key changes: (1) 14 shared components with TypeScript + Framer Motion hover animations. (2) Shell → Sidebar with 4 sections (Needs Attention/Active/Billing Soon/Archived) + TopNav with role switcher. (3) Dashboard → ARIAStrip + hero row (Ready to Bill CTA dominant) + 5 KPI cards + filter chips (All/Overdue/Lien Due/Ready to Bill/New Docs) + ARIA sort + Framer Motion stagger. (4) ProjectDetail → split-screen 55%/45%, left=financial scrollable, right=orbital sticky + ARIA insights always visible, VendorDetailPanel slide-in on planet click. (5) VendorDashboard → full orange #ea6c00 theme, VendorProjectCard status variants (needs_upload/approved/rejected/paid), VendorUploadModal, VendorTrustScore. (6) HubInbox → table view (checkbox/trade/doctype/amount/source/trust/date/status/actions), ARIA batch suggest banner, rejection quick-chips, filter tabs, pagination. (7) magic-upload.html + join.html → light theme #f8fafc with orange accent. (8) Bug fixes: onboarding→settings sync, PaymentPage money formatting, NotFound.tsx, hub upload emails to GC. (9) Zod schemas + safeValidate() for all API responses. (10) MSW handlers + Vitest Layer 7 = 211 component tests catching API shape crashes. |
