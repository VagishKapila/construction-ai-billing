...
// POST /api/admin/recalculate-project/:projectId
router.post('/recalculate-project/:projectId', adminAuth, async (req, res) => {
  ...
  try {
    ...
    // Get all submitted pay apps for this project (ordered by app_number)
    ...
    for (const pa of payApps.rows) {
      ...
      if (pa.is_retainage_release) {
        // ── Retainage release: calculate total retainage held across ALL prior pay apps ──
        // This is how much retainage was being held before the release
        ...
        // Also fix period label to include exact date if it's missing
        ...
        results.push({
          ...
          new_amount_due: totalRetainage,
          old_retention: oldRetention,
          new_retention: 0,
          fixed: oldAmountDue !== totalRetainage
        });
        ...
        results.push({
          ...
          old_amount_due: oldAmountDue,
          new_amount_due: totalAmountDue,
          old_retention: oldRetention,
          new_retention: retentionHeld,
