...
router.get('/payer-patterns', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE p.user_id = $1
        AND pa.submitted_at IS NOT NULL
      GROUP BY p.owner, p.owner_email
...
/**
 * POST /api/collection/followup-draft/:payAppId
 * Generate an AI follow-up email draft using Claude Haiku
 * Does NOT send automatically — user reviews and can copy/paste or edit
 *
 * Returns:
 * {
 *   ok: true,
 *   draft: "Follow-up email text...",
 *   payAppId: INTEGER,
 *   appNumber: INTEGER
 * }
 */
router.post('/followup-draft/:payAppId', async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    if (!payAppId) {
      return res.status(400).json({ error: 'Invalid pay app ID' });
    }

    // Fetch pay app with project details
    const result = await pool.query(`
      SELECT
        pa.id,
        pa.pay_app_number,
      ...
      JOIN projects p ON pa.project_id = p.id
      WHERE pa.id = $1 AND p.user_id = $2
    `, [payAppId, req.user.id]);

    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Pay app not found' });
    }

    const payApp = result.rows[0];

    // Import collection tracker
    const collectionTracker = require('../features/collection-tracker');

    // Generate follow-up draft
    const draft = await collectionTracker.generateFollowUpDraft(payApp);
    ...
    console.error('[Collection API] /followup-draft error:', e.message);
    res.status(500).json({ error: e.message || 'Failed to generate follow-up draft' });
  }
});

/**
 * POST /api/collection/followup-record/:payAppId
 * Record a follow-up (draft review, email sent, etc.) in the audit trail
 *
 * Body:
 * {
 *   followup_type: 'ai_draft' | 'email_sent' | 'phone_call' | 'note',
 *   notes: STRING (optional — the follow-up text or notes)
 * }
 */
router.post('/followup-record/:payAppId', async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    if (!payAppId || !followup_type) {
      return res.status(400).json({ error: 'Missing required fields: followup_type' });
    }

    const validTypes = ['ai_draft', 'email_sent', 'phone_call', 'note'];
    if (!validTypes.includes(followup_type)) {
      return res.status(400).json({ error: `Invalid followup_type. Must be one of: ${validTypes.join(', ')}` });
    }
    ...
    `, [payAppId, req.user.id, followup_type, notes || null]);
    ...
    res.json({
      ok: true,
      followup,
      message: `Follow-up recorded: ${followup_type}`
    });
  } catch (e) {
    console.error('[Collection API] /followup-record error:', e.message);
