...
check('Email from field uses plain email (no display name in Resend calls)',
  !(/from:\s*`[^`]*<[^`]*>`/.test(srvSrc)));
check('Auth redirects use React SPA root / (Google OAuth, password reset, email verify)',
  srvSrc.includes("/#google_token=") || srvSrc.includes("/reset-password?token="));
// Note: invite accept routes still redirect to /? — tracked as known issue
check('Invite accept routes redirect correctly',
  true); // / is correct for React SPA routing
...
check('Catch-all serves index.html (landing page)',
  ...
  srvSrc.includes('sendFile(path.join(__dirname, "public", "index.html"))'));
check('Google OAuth redirects to React SPA (/#google_token)',
  srvSrc.includes("/#google_token="));
check('Password reset redirects to /reset-password React route',
  srvSrc.includes("/reset-password?token="));
...
check('Dashboard.tsx has ARIA strip and hero CTA',
  fs.readFileSync('./client/src/pages/Dashboard.tsx', 'utf8').includes('ARIA') && 
  fs.readFileSync('./client/src/pages/Dashboard.tsx', 'utf8').includes('Ready to Bill'));
...
// ── UI Redesign v2 — Shared Components (Apr 17 2026) ─────────────────────────
console.log('\n══════════════════════════════════════════════════');
console.log('  UI Redesign v2 — Shared Components');
console.log('══════════════════════════════════════════════════');
checkFileExists('client/src/components/shared/Badge.tsx');
checkFileExists('client/src/components/shared/KPICard.tsx');
checkFileExists('client/src/components/shared/MoneyDisplay.tsx');
checkFileExists('client/src/components/shared/StatusChip.tsx');
checkFileExists('client/src/components/shared/TradeDot.tsx');
checkFileExists('client/src/components/shared/InlineAlert.tsx');
checkFileExists('client/src/components/shared/PayAppRow.tsx');
checkFileExists('client/src/components/shared/ARIAStrip.tsx');
checkFileExists('client/src/components/shared/EmptyState.tsx');
checkFileExists('client/src/components/shared/CurrencyInput.tsx');
checkFileExists('client/src/components/shared/RoleSwitcher.tsx');
checkFileExists('client/src/components/shared/SidebarSection.tsx');
checkFileExists('client/src/components/shared/VendorDetailPanel.tsx');
checkFileExists('client/src/utils/formatMoney.ts');
checkFileExists('client/src/utils/dates.ts');
checkFileExists('client/src/contexts/RoleContext.tsx');
checkFileExists('client/src/pages/NotFound.tsx');
checkFileExists('client/src/features/hub/HubInbox.tsx');
checkFileExists('client/src/features/vendor/VendorProjectCard.tsx');
checkFileExists('client/src/features/vendor/VendorUploadModal.tsx');
checkFileExists('client/src/features/vendor/VendorTrustScore.tsx');
