...
${pa.special_notes ? `<div style="margin-top:8px;padding:6px 10px;background:#fafafa;border:1px solid #ddd;border-radius:4px;font-size:8pt;color:#333"><strong>Notes &amp; Conditions:</strong> ${pa.special_notes}</div>` : ''}
...
app.get('/api/payapps/:id/pdf', async (req,res) => {
  ...
  if(pa.special_notes){doc.moveDown(0.5);doc.font('Helvetica').fontSize(8).text('Notes & Conditions: '+pa.special_notes,L,doc.y,{width:500});}
    ...
    if (emailAttachments.length === 0) {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          if (pa.special_notes) { pdfDoc.moveDown(0.5); pdfDoc.font('Helvetica').fontSize(8).text(`Notes & Conditions: ${pa.special_notes}`); }
          pdfDoc.moveDown(1);
