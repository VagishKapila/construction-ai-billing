...
<div class="aia-header">
  ...
  <div class="aia-appnum">
    ...
    ${pa.po_number ? `<div style="margin-top:2px">PO #: <strong>${pa.po_number}</strong></div>` : ''}
  </div>
</div>
...
${pa.special_notes ? `<div style="margin-top:8px;padding:6px 10px;background:#fafafa;border:1px solid #ddd;border-radius:4px;font-size:8pt;color:#333;white-space:pre-line"><strong>Notes:</strong> ${pa.special_notes}</div>` : ''}
...
app.get('/api/payapps/:id/pdf', async (req,res) => {
  ...
  if(pa.special_notes){doc.moveDown(0.5);doc.font('Helvetica-Bold').fontSize(8).text('Notes:',L,doc.y,{continued:true});doc.font('Helvetica').text(' '+pa.special_notes,{width:500});}
    ...
    if (emailAttachments.length === 0) {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          if (pa.special_notes) { pdfDoc.moveDown(0.5); pdfDoc.font('Helvetica-Bold').fontSize(8).text('Notes:',{continued:true}); pdfDoc.font('Helvetica').text(' '+pa.special_notes); }
          pdfDoc.moveDown(1);
